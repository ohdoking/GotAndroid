				...
				if (rect.contains(x, y)) {
					...
					if (mDownView_parent.getChildCount() == 1) {
						...
						if (SwipeType == Dismiss) {
							...
							HalfDrawable = activity.getResources().getDrawable(R.drawable.ic_launcher);
						}
...
//			mDownView_parent_txt2.setId(222222);
			...
			mDownView_parent_txt2.setWidth((textwidth2/3));
	...
	private void FullSwipeTrigger() {
		...
		mDownView.animate().translationX((float) -(width/a)).setDuration(300).setListener(new AnimatorListenerAdapter() {
			@Override
			public void onAnimationEnd(Animator animation) {
				super.onAnimationEnd(animation);
	...
	private void performDismiss(final View dismissView, final int dismissPosition) {
		...
		((ViewGroup) dismissView).getChildAt(1).animate().translationX(0).alpha(1f)
